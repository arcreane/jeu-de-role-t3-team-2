...
class MessageBox(object):
    ...
    def __init__(self, msg, b1, b2, parent, cbo, cboList):
        ...
        if cbo:
            self.cbo = ttk.Combobox(frm_1, state="readonly", justify="center", values= cboList, width=400)
            self.cbo.pack()
            self.cbo.focus_set()
            self.cbo.current(0)
        # button frame
        ...
        #root.geometry("400x110+%d+%d" % (parent.winfo_rootx()+7, parent.winfo_rooty()+70))

        positionRight = int(root.winfo_screenwidth() / 2 - 400 / 2)
        positionDown = int(root.winfo_screenheight() / 2 - 110 / 2)
